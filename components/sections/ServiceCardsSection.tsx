        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6">
          {cards.map((card, idx) => (
            <FadeIn key={idx} delay={idx * 0.1}>
              <div 
                className="group flex flex-col h-full bg-noir-surface rounded-[16px] md:rounded-[24px] overflow-hidden border border-[rgba(212,175,55,0.12)] hover:border-gold-primary/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl hover:shadow-gold-primary/5"
              >
              <div className="relative w-full aspect-[4/3] bg-noir-card overflow-hidden flex items-center justify-center">
                <Image
                  src={card.image}
                  alt={card.title}
                  fill
